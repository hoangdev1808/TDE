...
function createPagination() {
    ...
    for (let i = 0; i < totalSlideNumber; i++) {
        ...
        if (i == 0) {
            $("#nav-pagination ul").append(
                "<li class='active' data-section='" + (i + 1) + "'>" + (i + 1) + spanHtml + "</li>"
            );
        } else {
            $("#nav-pagination ul").append(
                "<li data-section='" + (i + 1) + "'>" + (i + 1) + spanHtml + "</li>"
            );
        }
    }
    $("#nav-pagination ul li").on("click", function () {
        ...
        if (e < currentSlideNumber) {
            ...
            $(".full-page section:nth-child(" + (currentSlideNumber + 1) + ")").addClass(
                "up-scroll current-page"
            );
...
function backToTop() {
    if ($(window).outerWidth() > 1025) {
        $(".back-to-top").on("click", function () {
            ...
            if (e < currentSlideNumber) {
                currentSlideNumber = e;
                $(".page-wrap section:nth-child(" + (currentSlideNumber + 1) + ")").addClass(
                    "up-scroll current-page"
                );
                $(".page-wrap section").removeClass("down-scroll");
                $(`#nav-pagination ul li`).removeClass("active");
                $(`#nav-pagination ul li[data-section=${currentSlideNumber + 1}]`).addClass(
                    "active"
                );
                for (let i = currentSlideNumber; i < totalSlideNumber; i++) {
                    $(".page-wrap section:nth-child(" + (currentSlideNumber + 1) + ")").addClass(
